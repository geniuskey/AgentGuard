...
| **Default Deny** | 켜면(`defaultMode: dontAsk`) 명시적으로 Allow한 경로 외에는 전부 차단 — 사내 보안 환경 권장 |
...
- 일치 규칙이 없으면 Default Deny 여부에 따라 차단 또는 확인 요청
...
| Default Deny | 활성 Scope의 `defaultMode: dontAsk` 토글 (켜짐 = 녹색 = 보호 활성) |
