...
> **개정:** 초기 v0.1의 "Default Deny 모드(`defaultMode: dontAsk`)"는 폐기되었다. 모드 토글로
> 관리돼 쉽게 벗어날 수 있어 보안 의미가 약했기 때문이다. 정책은 명시적 경로 규칙만으로 통제한다.

- 민감한 폴더·파일은 드라이브/폴더 단위로 명시적으로 **Deny** 처리한다.
- 필요한 작업 폴더만 명시적으로 **Allow** 처리한다.
- 불확실한 경로는 **Ask**로 두어 사용자 확인 후 접근하게 한다.
...
- **프로젝트를 열지 않고도** Home에서 바로 User settings(`~/.claude/settings.json`)를 편집할 수 있어야 한다. 프로젝트 선택 전 기본 편집 대상은 User scope다.
- User scope는 프로젝트 파일 트리가 없으므로, 경로/패턴을 직접 추가하는 **규칙 목록(Rule List) 편집기**와 **Raw JSON 편집기**를 함께 제공한다.
...
- 저장 전 Diff를 보여주고, 저장 시 자동 백업한다. (백업 파일명: `..._user-settings.json`)
- 기존 settings.json의 알 수 없는 필드와 비관리 규칙(`Bash(...)`, `WebFetch(...)` 등)은 보존한다.
...
> **개정:** "Default Deny + Allow Island"(`defaultMode: dontAsk`)는 폐기되었다. 프로필은 이제
> deny-by-default 없이 민감 경로 Deny + 소스 Allow 규칙만 적용한다(effective-policy.md §2).
...
- `.env*`: Deny 추천
- `secrets/**`: Deny 추천
- 그 외 규칙 없는 경로: Claude Code 기본 동작(실행 시 확인)
...
- 대상 파일 경로 표시: `~/.claude/settings.json`
- 규칙 / Raw JSON 모드 전환
...
- 홈 경로 패턴 예시 안내 (`~/.ssh/**`, `~/.aws/**` 등)
- Raw JSON: Validate / Format / Save
