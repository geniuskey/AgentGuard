...
1. 데스크톱 앱은 프로젝트 내용을 **외부로 전송하지 않으며** 텔레메트리를 두지 않는다
   (Local First). 개발자용 Claude permission probe는 사용자가 명시적으로 실행하는 별도
   스크립트이며, 인증된 Claude CLI의 네트워크/API 사용이 발생할 수 있다.
2. 앱은 Secret 값을 별도 수집하거나 DB 필드에 저장하지 않는다. 단 사용자가 편집하는 원본
   Claude 설정에 Secret이 있으면 보존 파싱과 저장 전 백업 때문에 백업 파일에도 남을 수 있다.
   따라서 앱 데이터 디렉터리 역시 사용자 전용 민감 데이터로 취급한다.
...
9. 앱 내부 DB에는 메타데이터만 저장한다. `backups/`의 설정 원문 복사본은 이 원칙과 별개다.
...
- 앱에는 원격 telemetry/crash upload가 없고 현재 영구 진단 로그도 만들지 않는다.
- `app.db`와 백업에는 프로젝트 경로·규칙·환경 설정이 포함될 수 있으므로 공개 Issue에
  원본을 첨부하지 않는다. 정제 절차는 [diagnostics-and-privacy.md](diagnostics-and-privacy.md).
- npm/Cargo/GitHub Actions 업데이트는 Dependabot이 주간 확인하고, Cargo.lock은 RustSec
  advisory database로 PR/push 및 매주 검사한다.
- 서명되지 않은 prerelease와 코드 서명된 production artifact를 구분한다. 인증서·timestamp,
  수동 업데이트·롤백의 운영 조건은 [release-policy.md](release-policy.md)에 따른다.
- 보안 취약점은 공개 Issue가 아니라 [`SECURITY.md`](../SECURITY.md)의 비공개 채널로 제보한다.
