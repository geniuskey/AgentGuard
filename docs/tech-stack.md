...
- **Monaco**: Raw JSON 하이라이트/검증/포맷의 사실상 표준.
...
4. **Monaco는 Raw JSON 탭 한 곳뿐** — Svelte에서 작은 mount 래퍼로 격리 통합하면 되므로
   React의 "Monaco 래퍼 풍부" 장점이 사실상 무의미하다.
...
- Monaco는 `RawJsonEditor.svelte`에서 `onMount` 시 동적 import로 로드(초기 번들 경량 유지).
